...
class AdManagerSCARSnippets {
  ...
  fun loadNativePlusBanner(context: Context, adUnitID: String) {
    ...
    QueryInfo.generate(
      context,
      AdFormat.NATIVE, // Use Native ad format for Native + Banner.
      adRequest,
      adUnitID,
      object : QueryInfoGenerationCallback() {
        override fun onSuccess(queryInfo: QueryInfo) {
  ...
  fun renderNative(context: Context, adUnitID: String, adResponseString: String) {
    // [START render_native]
    val adRequest = AdManagerAdRequest.Builder().setAdString(adResponseString).build()
  ...
  fun renderNativePlusBanner(context: Context, adUnitID: String, adResponseString: String) {
